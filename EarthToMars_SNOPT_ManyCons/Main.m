...
fileG='../EarthToMars_SNOPT_ManyCons/GMAT_thrust.txt';
fIDG=fopen(fileG,'r');
...
x=[GMAT_data.Thrustx(2:(end-1));GMAT_data.Thrusty(2:(end-1));GMAT_data.Thrustz(2:(end-1));GMAT_data.TOF/TU];


% Bounds
lb = [-ones(NumberOfSteps,1)*Th;    % x
      -ones(NumberOfSteps,1)*Th;    % y
      -ones(NumberOfSteps,1)*Th;    % z
      10*86400/TU];             % TOF (TU)
  
ub = [ones(NumberOfSteps,1)*Th;     % x 
      ones(NumberOfSteps,1)*Th;     % y
      ones(NumberOfSteps,1)*Th;    % z
      3500*86400/TU];            % TOF (TU)
     %3500 works
...
Flow=zeros((NumberOfSteps+7), 1);
Fupp=zeros((NumberOfSteps+7), 1);
Flow(1) = 0;
Fupp(1) = ub(end);%inf;
%bounds of constraints
Flow(2:7) = 0;
Fupp(2:7) = 0;
Flow(8:(NumberOfSteps+7))=Th;
Fupp(8:(NumberOfSteps+7))=Th;
...
Fmul = zeros((NumberOfSteps+7), 1); %Lagrange multipliers
Fstate = zeros((NumberOfSteps+7), 1);

% Directory Path to create Temp Files
my_dir = 'C:/GMAT_Repo/EarthToMars_SNOPT_ManyCons';
...
snseti('Major iteration limit', 500);%5000);
