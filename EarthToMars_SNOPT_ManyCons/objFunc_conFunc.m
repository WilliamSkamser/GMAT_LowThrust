...
TOF     = x(end)*TU;                                           % s
%Th.*[cos(Thrust_beta).*cos(Thrust_alpha),cos(Thrust_beta).*sin(Thrust_alpha),sin(Thrust_beta)];
%% Create Thrust History 
Thrust = zeros(NumberOfSteps+1,3);
Thrust(1:(end-1),1) = x(1:NumberOfSteps);
Thrust(1:(end-1),2) = x(NumberOfSteps+1:2*NumberOfSteps);
Thrust(1:(end-1),3) = x((2*NumberOfSteps)+1:3*NumberOfSteps);
...
% Temporary File
%file2 = tempname(
file2='C:/GMAT_Repo/EarthToMars_SNOPT_ManyCons/ThrustProfile.thrust';
%file2 = [file2,'.thrust'];
...
% Load GMAT SCRIPT
gmat.gmat.LoadScript("C:/GMAT_Repo/EarthToMars_SNOPT_ManyCons/GMATScriptEarthMars.script");
...
Mars_Z=Data(14);
%}
Mag=zeros(NumberOfSteps,1); 
for i=1:NumberOfSteps
    Mag(i,1)=norm([Thrust(i,1),Thrust(i,2),Thrust(i,3)]);
end
